...
`.trim();
  ...
  function hiddenNobleTypeWiki(result) {
    const labels = [];
    ...
    result.input.contentSettings.noble.variants.forEach((variant) => {
      ...
      labels.push(NP_TYPE[variant.npType].wiki.replace(/^BGCOLOR\([^)]*\):/, ""));
    });
    return `${result.npType.wiki}${labels.map((label) => `／${label}`).join("")}`;
  }
  ...
  function replaceSource(source, result) {
    ...
    text = replaceLine(text,
      ...
      (line) => {
        ...
        cells[cells.length - 2] = hiddenNobleTypeWiki(result);
        return cells.join("|");
  ...
  const core = {
    ...
    createDefaultContentSettings, normalizeContentSettings, buildOwnedSkillsSection, buildNobleSection, formatEffectSuffix, hiddenNobleTypeWiki,
  ...
  function effectPreviewLine(effect) {
    ...
    const text = `${item.prefix}${cleanInline(item.text)}`;
  ...
  function durationOptions() {
    return [["1", "1"], ["2", "2"], ["3", "3"], ["5", "5"], ["10", "10"], ["custom", "自由入力"]];
  }

  function durationControl(kind, label) {
    return `<label class="fsc-duration-field"><span>${label}</span><select data-duration-select="${kind}">${optionList(durationOptions(), "1")}</select>
      <input type="number" min="1" step="1" value="1" data-duration-custom="${kind}" aria-label="${label}の自由入力" hidden></label>`;
  }
  ...
  function effectDurationTools(path) {
    return `<div class="fsc-duration-tools">
      ...
      <div class="fsc-duration-buttons">
        <button type="button" class="fsc-mini" data-content-action="appendDuration" data-duration-format="turn" data-target-path="${path}.text">(1T)を追加</button>
        <button type="button" class="fsc-mini" data-content-action="appendDuration" data-duration-format="count" data-target-path="${path}.text">(1回)を追加</button>
        <button type="button" class="fsc-mini" data-content-action="appendDuration" data-duration-format="both" data-target-path="${path}.text">(1回・1T)を追加</button>
      </div>
    </div>`;
  }
  ...
  function effectEditor(effect, path, listPath, index, total, kind) {
    ...
    </div>${effectDurationTools(path)}${count ? `<div class="fsc-grid fsc-values">${valueFields}</div>` : ""}`;
    ...
    return `<div class="fsc-effect-row fsc-effect-tone-${index % 3}">
  ...
  function ownedSkillEditor(skill, index) {
    ...
    return `<details class="fsc-entry fsc-entry-tone-${index % 4}" open><summary>Skill${index + 1}</summary>
  ...
  function classSkillEditor(skill, index, total) {
    ...
    return `<details class="fsc-entry fsc-entry-tone-${index % 4}" open><summary>クラススキル${index + 1}</summary><div class="fsc-entry-body">
  ...
  function renderContentSettingsEditors(settings) {
    return {
      ...
      noble: `<details class="fsc-entry fsc-entry-tone-3" open><summary>宝具</summary><div class="fsc-entry-body">${nobleVersionEditor(settings.noble.base, "noble.base", "通常")}
      ...
      bond: `<details class="fsc-entry fsc-entry-tone-0" open><summary>絆礼装</summary><div class="fsc-entry-body">${effectPreview(settings.bond.effects, "bond.effects")}<div class="fsc-grid">
  ...
  function installStyle() {
    ...
    style.textContent = `
      ...
      @media(max-width:700px){.fsc-shell{padding:10px}.fsc-split{grid-template-columns:1fr}.fsc-section{padding:10px}.fsc-command-cards,.fsc-command-hits{grid-template-columns:repeat(2,minmax(120px,1fr))}.fsc-duration-tools{grid-template-columns:1fr 1fr}.fsc-duration-buttons{grid-column:1/-1} }
    `;
  ...
  function renderApp(root) {
    ...
    root.innerHTML = `<form class="fsc-shell" autocomplete="on">
      ...
      <section class="fsc-section"><h3>宝具設定</h3>
        <p class="fsc-help">宝具は1枠です。通常・強化後と、第二／第三再臨以降の名称・効果・宝具種類の差分を設定します。通常の宝具種類は、上の「コマンドカード・Hit数」で選択した内容を使用します。</p>
    ...
    root.addEventListener("click", (event) => {
      ...
      if (action === "addEffect") {
        ...
        const selectedValue = (kind) => {
          if (!row) return "";
          const selector = row.querySelector(`[data-duration-select="${kind}"]`);
          if (!selector) return "";
          if (selector.value !== "custom") return selector.value;
        ...
        const resolved = modelTarget(button.dataset.targetPath);
        if (resolved && token) resolved.target[resolved.key] = `${stringValue(resolved.target[resolved.key])}${token}`;
      } else return;
      renderContentEditors();
      saveState();
      refresh();
    ...
    root.addEventListener("change", (event) => {
      ...
      if (event.target.matches("[data-duration-select]")) {
        const row = event.target.closest(".fsc-effect-row");
        const custom = row && row.querySelector(`[data-duration-custom="${event.target.dataset.durationSelect}"]`);
        if (custom) {
