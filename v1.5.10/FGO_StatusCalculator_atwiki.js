(function () {
  ...
  const SKILL_ICON_PAGE_URL = "https://w.atwiki.jp/siroi_human/pages/20.html";
...
`.trim();
  ...
  function normalizeGenderEntries(value, legacyGender) {
    const source = Array.isArray(value) ? value : [];
    const entries = source.map((item) => {
      const object = item && typeof item === "object" ? item : {};
      const candidate = typeof item === "string" ? item : (object.gender || object.value);
      return {
        gender: GENDERS.includes(candidate) ? candidate : GENDERS[0],
        note: stringValue(object.note).trim()
      };
    });
    if (!entries.length) {
      entries.push({ gender: GENDERS.includes(legacyGender) ? legacyGender : GENDERS[0], note: "" });
    }
    return entries;
  }

  function formatGenderEntry(value) {
    const entry = normalizeGenderEntries([value])[0];
    const note = cleanInline(entry.note).replace(/\|/g, "／");
    return note ? `${entry.gender}&footnote(${note})` : entry.gender;
  }

  function emptyEffect() {
    return {
      prefix: "", text: "", valueMode: "none", values: [], raw: false, rawCode: "",
      effectType: "normal", attackTarget: "auto", attackStrength: "base", specialTarget: "", chance: ""
    };
  }

  function normalizeEffectChance(value) {
    const source = cleanInline(value).replace(/[％%]$/, "");
    if (!/^\d+$/.test(source)) return "";
    const chance = Number(source);
    return Number.isFinite(chance) && chance >= 0 ? String(chance) : "";
  }

  function formatEffectChance(value) {
    const chance = normalizeEffectChance(value);
    if (!chance) return "";
    return Number(chance) >= 101
      ? ` &color(#FF0000){確率${chance}％}`
      : ` 確率${chance}％`;
  }
  ...
  function normalizeEffect(effect) {
    ...
    return {
      ...
      chance: normalizeEffectChance(source.chance)
    };
  }
  ...
  function createDefaultContentSettings() {
    return {
      ...
      bond: { name: "", icon: "", includeFieldCondition: true, effects: [] }
    };
  }
  ...
  function normalizeContentSettings(settings) {
    ...
    return {
      ...
      bond: {
        ...
        includeFieldCondition: source.bond && Object.prototype.hasOwnProperty.call(source.bond, "includeFieldCondition")
          ? Boolean(source.bond.includeFieldCondition)
          : defaults.bond.includeFieldCondition,
        effects: normalizeEffects(source.bond && source.bond.effects)
      }
    };
  }

  function preUpgradeEffects(settings, enabledPath) {
    const normalized = normalizeContentSettings(settings);
    const path = String(enabledPath || "");
    let sourceEffects = [];
    let match = path.match(/^skills\.(\d+)\.upgraded\.enabled$/);
    if (match) {
      const skill = normalized.skills[Number(match[1])];
      sourceEffects = skill ? skill.base.effects : [];
    } else {
      match = path.match(/^skills\.(\d+)\.variants\.(\d+)\.upgraded\.enabled$/);
      if (match) {
        const skill = normalized.skills[Number(match[1])];
        const variant = skill && skill.variants[Number(match[2])];
        sourceEffects = skill && variant ? resolvedSkillVariant(skill.base, variant).effects : [];
      } else if (path === "noble.upgraded.enabled") {
        sourceEffects = normalized.noble.base.effects;
      } else {
        match = path.match(/^noble\.variants\.(\d+)\.upgraded\.enabled$/);
        if (match) {
          const variant = normalized.noble.variants[Number(match[1])];
          sourceEffects = variant ? resolvedNobleVariant(normalized.noble.base, variant).effects : [];
        }
      }
    }
    return normalizeEffects(sourceEffects);
  }

  function replaceUpgradedEffects(settings, enabledPath) {
    const path = String(enabledPath || "");
    if (!/^(?:skills\.\d+(?:\.variants\.\d+)?|noble(?:\.variants\.\d+)?)\.upgraded\.enabled$/.test(path)) return false;
    const effectsPath = path.replace(/\.enabled$/, ".effects");
    const parts = effectsPath.split(".");
    let target = settings;
    for (let index = 0; index < parts.length - 1; index += 1) {
      if (!target || typeof target !== "object" || !Object.prototype.hasOwnProperty.call(target, parts[index])) return false;
      target = target[parts[index]];
    }
    if (!target || typeof target !== "object") return false;
    target[parts[parts.length - 1]] = preUpgradeEffects(settings, path);
    return true;
  }
  ...
  function calculate(raw) {
    ...
    const genders = normalizeGenderEntries(raw.genders, raw.gender);
    const gender = genders[0].gender;
  ...
  function buildTraitLine(line, result) {
    ...
    const genderTraits = normalizeGenderEntries(result.input.genders, result.input.gender)
      .map((entry) => entry.gender === "-" ? "性別不明" : entry.gender)
      .filter((trait, index, traits) => traits.indexOf(trait) === index);
    const derived = genderTraits.concat(result.input.policy);
    ...
    cells[traitCellIndex] = `LEFT:${formatted.join(" / ")}`;
    return cells.join("|");
  }
  ...
  function stripCornerBrackets(value) {
    return cleanInline(value).replace(/^【\s*/, "").replace(/\s*】$/, "").trim();
  }

  function skillNameWithoutRank(value) {
    return stripCornerBrackets(value)
      .replace(/\s+(?:EX|[A-E](?:\+{1,3}|-)?)(?=\s*(?:〔[^〕]+〕)?$)/i, "")
      .trim();
  }
  ...
  function ascensionRangeLabels(variants) {
    const enabledStages = new Set((Array.isArray(variants) ? variants : [])
      .filter((variant) => variant && variant.enabled)
      .map((variant) => String(variant.stage)));
    const hasSecond = enabledStages.has("2");
    const hasThird = enabledStages.has("3");
    if (!hasSecond && !hasThird) return { base: "", 2: "", 3: "" };
    return {
      base: hasSecond ? "第一再臨時" : "第一・第二再臨時",
      2: hasSecond ? (hasThird ? "第二再臨時" : "第二・第三再臨時") : "",
      3: hasThird ? "第三再臨時" : ""
    };
  }

  function ascensionHeadingSuffix(label) {
    return label ? `[${label}]` : "";
  }
  ...
  function effectHasData(effect) {
    ...
    return Boolean(item.effectType !== "normal" || cleanInline(item.text) || cleanInline(item.rawCode)
      || (!item.raw && item.chance) || item.values.some((value) => cleanInline(value)));
  }
  ...
  function buildSkillEffectRows(version) {
    ...
    effects.forEach((effect, index) => {
      ...
      const text = `${effect.prefix}${cleanInline(effect.text)}${formatEffectChance(effect.chance)}`;
  ...
  function buildSkillTable(slot, version, headingLabel) {
    const icon = normalizeIconFile(cleanInline(version.icon) || skillNameWithoutRank(version.name));
    return [
  ...
  function buildOwnedSkill(skill) {
    ...
    const enabledVariants = skill.variants.filter((variant) => variant.enabled);
    const rangeLabels = ascensionRangeLabels(enabledVariants);
    const baseSuffix = ascensionHeadingSuffix(rangeLabels.base);
    const parts = [buildSkillTable(slot, base, baseSuffix)];
    enabledVariants.forEach((variant) => {
      const rangeLabel = rangeLabels[String(variant.stage)] || ascensionLabel(variant.stage);
      const headingSuffix = ascensionHeadingSuffix(rangeLabel);
      const resolved = resolvedSkillVariant(base, variant);
      const changesEffects = ["nameEffect", "all"].includes(variant.changeMode);
      if (!changesEffects) parts.push(`#region(close,${rangeLabel})`);
      if (variant.changeMode === "name") parts.push(`***Skill${slot}${headingSuffix}：${cleanInline(resolved.name)}`);
      else parts.push(buildSkillTable(slot, resolved, headingSuffix));
      ...
      if (!changesEffects) parts.push("#endregion()");
    });
  ...
  function standardNobleAttack(effect, npTypeKey, omitTarget) {
    ...
    const targetText = omitTarget
      ? (target === "single" ? "＆超強力な攻撃[Lv]" : "＆強力な攻撃[Lv]")
      : (target === "single" ? "敵単体に超強力な攻撃[Lv]" : "敵全体に強力な攻撃[Lv]");
    return {
  ...
  function effectAttackTarget(effect, npTypeKey) {
    if (!effect || effect.raw) return "";
    if (effect.effectType === "nobleAttack") return nobleAttackTarget(effect, npTypeKey);
    if (effect.effectType !== "normal") return "";
    const text = `${effect.prefix || ""}${cleanInline(effect.text)}`.replace(/^[＆＋\s　]+/, "");
    if (/^敵全体(?:に|の|へ)/.test(text)) return "all";
    if (/^敵単体(?:に|の|へ)/.test(text)) return "single";
    return "";
  }
  ...
  function buildNobleEffectRows(version, result, npTypeKey) {
    ...
    effects.forEach((effect, index) => {
      ...
      if (effect.effectType === "nobleAttack") {
        const currentTarget = nobleAttackTarget(effect, npTypeKey);
        const previousTarget = index > 0 ? effectAttackTarget(effects[index - 1], npTypeKey) : "";
        const attack = standardNobleAttack(effect, npTypeKey, previousTarget === currentTarget);
        rows.push(`|${lead.concat(`${attack.text}${formatEffectChance(effect.chance)}`, attack.cells).join("|")}|`);
  ...
  function buildNobleSection(settings, result) {
    ...
    const enabledVariants = settings.noble.variants.filter((variant) => variant.enabled);
    const rangeLabels = ascensionRangeLabels(enabledVariants);
    const parts = ["**宝具"];
    if (rangeLabels.base) parts.push(`***宝具${ascensionHeadingSuffix(rangeLabels.base)}`);
    parts.push(buildNobleTable(base, result, result.input.npType));
    enabledVariants.forEach((variant) => {
      const rangeLabel = rangeLabels[String(variant.stage)] || ascensionLabel(variant.stage);
      ...
      const changesEffects = variant.changeMode === "nameEffect";
      if (!changesEffects) parts.push(`#region(close,${rangeLabel})`);
      parts.push(`***宝具${ascensionHeadingSuffix(rangeLabel)}`);
  ...
  function buildClassSkillRows(skill) {
    ...
    effects.forEach((effect) => {
      ...
      else rows.push(`|~|${effect.prefix}${cleanInline(effect.text)}${formatEffectChance(effect.chance)}|${cleanInline(effect.values[0])}|`);
    });
  ...
  function buildClassSkillSection(settings, result) {
    ...
    const blocks = skills.map((skill) => [
      ...
      `|&ref(${normalizeIconFile(skill.icon)},icon/skill,height=48)|>|BGCOLOR(#e6e6fa):CENTER:&font(b,110%){${stripCornerBrackets(skill.name)}}${result.classSkillIcon}|`,
  ...
  function buildBondRows(bond, result) {
    ...
    const owner = `${cleanInline(result.input.trueName)}〔${cleanInline(result.input.classKey)}〕`;
    const fieldCondition = bond.includeFieldCondition ? "自身がフィールドにいる間、" : "";
    const preamble = `&font(,b,#00cc58){${owner}}装備時のみ、&br()${fieldCondition}`;
    if (!effects.length) {
    ...
    effects.forEach((effect, index) => {
      ...
      const text = `${index === 0 ? preamble : ""}${effect.prefix}${cleanInline(effect.text)}${formatEffectChance(effect.chance)}`;
  ...
  function replaceHiddenIdentityRows(text, result, report) {
    const pattern = /^\|BGCOLOR\(#e6e6fa\):成長\|.*\|スター発生率\|[^|\n]*\|$/m;
    const match = pattern.exec(text);
    if (!match) {
      report.missing.push("成長・方針・性格・性別・スター発生率");
      return text;
    }
    const afterBase = match.index + match[0].length;
    const oldAdditionalRows = /^(?:\n\|~\|~\|~\|~\|~\|[^|\n]*\|~\|~\|)*/.exec(text.slice(afterBase))[0];
    const genders = normalizeGenderEntries(result.input.genders, result.input.gender);
    const cells = match[0].split("|");
    cells[2] = result.input.growth;
    cells[4] = result.input.policy;
    cells[5] = result.input.personality;
    cells[6] = formatGenderEntry(genders[0]);
    cells[cells.length - 2] = format(result.sr, 1);
    const rows = [cells.join("|")].concat(genders.slice(1).map((entry) => `|~|~|~|~|~|${formatGenderEntry(entry)}|~|~|`));
    report.replaced.push("成長・方針・性格・性別・スター発生率");
    return `${text.slice(0, match.index)}${rows.join("\n")}${text.slice(afterBase + oldAdditionalRows.length)}`;
  }
  ...
  function replaceSource(source, result) {
    ...
    text = replaceHiddenIdentityRows(text, result, report);
    text = replaceLine(text,
      /^\|ヒット数\|.*\|スター集中度\|[^|\n]*\|$/m,
  ...
  const core = {
    ...
    createDefaultContentSettings, normalizeContentSettings, normalizeGenderEntries, formatGenderEntry, genderEntriesEditor, preUpgradeEffects, replaceUpgradedEffects, buildOwnedSkillsSection, buildNobleSection, normalizeEffectChance, formatEffectChance, formatEffectSuffix, insertEffectNotation, toggleEffectNotation, effectTokens, renderContentSettingsEditors,
    SKILL_ICON_PAGE_URL, normalizeSkillIconFilename, parseSkillIconCatalog, skillIconPicker, skillIconModalHtml,
  ...
  function genderEntriesEditor(entries) {
    const normalized = normalizeGenderEntries(entries);
    const rows = normalized.map((entry, index) => `<div class="fsc-gender-entry">
      <label class="fsc-field"><span>性別${index + 1}</span><select data-gender-index="${index}" data-gender-field="gender">${optionList(GENDERS.map((value) => [value, value]), entry.gender)}</select></label>
      <label class="fsc-field"><span>脚注（任意）</span><input type="text" data-gender-index="${index}" data-gender-field="note" value="${escapeHtml(entry.note)}" autocomplete="on"></label>
      <button type="button" class="fsc-mini danger" data-gender-action="remove" data-gender-index="${index}"${normalized.length === 1 ? " disabled" : ""}>削除</button>
    </div>`).join("");
    return `<div class="fsc-gender-list">${rows}</div><button type="button" class="fsc-btn sub fsc-add" data-gender-action="add">性別を追加</button>`;
  }
  ...
  function normalizeSkillIconFilename(src) {
    try {
      const url = new URL(src, SKILL_ICON_PAGE_URL);
  ...
  function cleanSkillIconCategory(value) {
  ...
  function parseSkillIconCatalog(html) {
    ...
    Array.from(targetTable.querySelectorAll("tr")).forEach((row) => {
      ...
      if (!images.length) {
        const text = cleanSkillIconCategory(row.textContent);
        if (text && text.length <= 40 && !/wiki構文|サンプル画像/i.test(text)) category = text;
        return;
      }
      images.forEach((image) => {
        const src = image.getAttribute("src") || "";
        const filename = normalizeSkillIconFilename(src);
        if (!filename || seen.has(filename)) return;
        seen.add(filename);
        let absolute = src;
        try { absolute = new URL(src, SKILL_ICON_PAGE_URL).href; } catch (error) {}
  ...
  function skillIconSelectedHtml(path, value) {
    const name = String(value == null ? "" : value);
    return `<div class="fsc-icon-selected" data-icon-selected-for="${escapeHtml(path)}">
      <div class="fsc-icon-selected-image" data-icon-selected-image></div>
      <div class="fsc-icon-selected-meta"><span class="fsc-icon-selected-label">選択中</span><strong data-icon-selected-name>${escapeHtml(name || "未選択")}</strong></div>
      <div class="fsc-icon-selected-actions">
        <button type="button" class="fsc-btn sub" data-skill-icon-open>アイコンを選択</button>
        <button type="button" class="fsc-btn sub fsc-icon-clear" data-skill-icon-clear>選択解除</button>
      </div>
    </div>`;
  }

  function skillIconPicker(path, label, value) {
    return `<div class="fsc-field fsc-skill-icon-field" data-skill-icon-picker-for="${escapeHtml(path)}">
      <span>${escapeHtml(label)}</span>
      <input type="hidden" data-model-path="${escapeHtml(path)}" value="${escapeHtml(value == null ? "" : value)}">
      ${skillIconSelectedHtml(path, value)}
    </div>`;
  }

  function skillIconModalHtml() {
    return `<div class="fsc-icon-modal" data-skill-icon-modal hidden>
      <div class="fsc-icon-modal-backdrop" data-skill-icon-close></div>
      <section class="fsc-icon-modal-panel" role="dialog" aria-modal="true" aria-label="スキルアイコン選択">
        <div class="fsc-icon-modal-head">
          <div><strong>スキルアイコン選択</strong><div class="fsc-icon-modal-current" data-skill-icon-modal-current>未選択</div></div>
          <button type="button" class="fsc-icon-modal-close" data-skill-icon-close aria-label="閉じる">×</button>
        </div>
        <div class="fsc-icon-picker-tools">
          <input type="search" data-skill-icon-filter placeholder="アイコン名を検索">
          <button type="button" class="fsc-btn sub fsc-icon-refresh" data-skill-icon-refresh>アイコン一覧を更新</button>
        </div>
        <div class="fsc-icon-picker-status" data-skill-icon-status>アイコン一覧を読み込んでいます…</div>
        <div class="fsc-icon-picker-catalog" data-skill-icon-catalog></div>
      </section>
    </div>`;
  }

  let skillIconCatalogPromise = null;
  let skillIconCatalogCache = null;
  let activeSkillIconPicker = null;
  let skillIconModalElement = null;
  let skillIconCatalogRefreshing = false;
  let skillIconEventsBound = false;

  function loadSkillIconCatalog() {
    if (skillIconCatalogCache) return Promise.resolve(skillIconCatalogCache);
    if (skillIconCatalogPromise) return skillIconCatalogPromise;
    if (typeof fetch !== "function") return Promise.resolve([]);
    skillIconCatalogPromise = fetch(SKILL_ICON_PAGE_URL, { credentials: "same-origin" })
      .then((response) => {
        if (!response.ok) throw new Error(`HTTP ${response.status}`);
        return response.text();
      })
      .then((html) => {
        skillIconCatalogCache = parseSkillIconCatalog(html);
        return skillIconCatalogCache;
      })
      .catch(() => []);
    return skillIconCatalogPromise;
  }

  function skillIconChoiceHtml(item, selected) {
    const active = item.filename === selected ? " is-selected" : "";
    return `<button type="button" class="fsc-icon-choice${active}" data-skill-icon-value="${escapeHtml(item.filename)}" title="${escapeHtml(item.filename)}">
      <img src="${escapeHtml(item.src)}" alt="" loading="lazy">
      <span>${escapeHtml(item.filename.replace(/\.png$/i, ""))}</span>
    </button>`;
  }

  function ensureSkillIconModal() {
    if (typeof document === "undefined") return null;
    if (skillIconModalElement && document.body.contains(skillIconModalElement)) return skillIconModalElement;
    const host = document.createElement("div");
    host.innerHTML = skillIconModalHtml();
    skillIconModalElement = host.firstElementChild;
    document.body.appendChild(skillIconModalElement);
    return skillIconModalElement;
  }

  function selectedSkillIconValue(picker) {
    const hidden = picker && picker.querySelector('input[type="hidden"][data-model-path]');
    return hidden ? hidden.value || "" : "";
  }

  function updateSkillIconPreview(picker, catalog) {
    if (!picker) return;
    const hidden = picker.querySelector('input[type="hidden"][data-model-path]');
    ...
    const item = (catalog || skillIconCatalogCache || []).find((entry) => entry.filename === selected);
    if (imageBox) imageBox.innerHTML = item ? `<img src="${escapeHtml(item.src)}" alt="">` : "";
  }

  function refreshSkillIconPreviews(root, catalog) {
    if (!root || !root.querySelectorAll) return;
    root.querySelectorAll("[data-skill-icon-picker-for]").forEach((picker) => updateSkillIconPreview(picker, catalog));
  }

  function renderSkillIconCatalog(catalog) {
    const modal = ensureSkillIconModal();
    if (!modal) return;
    ...
    if (!catalog || !catalog.length) {
      ...
      if (!skillIconCatalogCache || !skillIconCatalogCache.length) target.innerHTML = "";
      return;
    }
    const selected = selectedSkillIconValue(activeSkillIconPicker);
    ...
    target.innerHTML = Array.from(grouped.entries()).map(([category, items]) => `
      <section class="fsc-icon-category" data-icon-category>
        <h4>${escapeHtml(category)}</h4>
        <div class="fsc-icon-grid">${items.map((item) => skillIconChoiceHtml(item, selected)).join("")}</div>
      </section>`).join("");
    if (status) status.textContent = `${catalog.length}件のアイコンを読み込みました。`;
  }

  function updateSkillIconModalSelection() {
    const modal = ensureSkillIconModal();
    if (!modal) return;
    const selected = selectedSkillIconValue(activeSkillIconPicker);
  ...
  function filterSkillIconCatalog(query) {
    const modal = ensureSkillIconModal();
    if (!modal) return;
  ...
  function openSkillIconModal(picker) {
    const modal = ensureSkillIconModal();
    if (!modal || !picker) return;
    activeSkillIconPicker = picker;
    modal.hidden = false;
    document.documentElement.classList.add("fsc-icon-modal-open");
    const filter = modal.querySelector("[data-skill-icon-filter]");
    if (filter) filter.value = "";
    updateSkillIconModalSelection();
    const status = modal.querySelector("[data-skill-icon-status]");
    if (status) status.textContent = "アイコン一覧を読み込んでいます…";
    const target = modal.querySelector("[data-skill-icon-catalog]");
    if (target && !skillIconCatalogCache) target.innerHTML = "";
    loadSkillIconCatalog().then((catalog) => {
      if (!modal.hidden) {
        renderSkillIconCatalog(catalog);
        updateSkillIconModalSelection();
      }
      updateSkillIconPreview(activeSkillIconPicker, catalog);
    });
    if (filter) filter.focus();
  }

  function closeSkillIconModal() {
    if (!skillIconModalElement) return;
    skillIconModalElement.hidden = true;
    document.documentElement.classList.remove("fsc-icon-modal-open");
    activeSkillIconPicker = null;
  }

  function setSelectedSkillIcon(filename) {
    if (!activeSkillIconPicker) return;
    const hidden = activeSkillIconPicker.querySelector('input[type="hidden"][data-model-path]');
    if (!hidden) return;
    hidden.value = filename || "";
    hidden.dispatchEvent(new Event("change", { bubbles: true }));
    updateSkillIconPreview(activeSkillIconPicker, skillIconCatalogCache || []);
    updateSkillIconModalSelection();
  }

  function refreshSkillIconCatalog() {
    const modal = ensureSkillIconModal();
    if (!modal || skillIconCatalogRefreshing || typeof fetch !== "function") return Promise.resolve(skillIconCatalogCache || []);
    const button = modal.querySelector("[data-skill-icon-refresh]");
    const status = modal.querySelector("[data-skill-icon-status]");
    const filter = modal.querySelector("[data-skill-icon-filter]");
    skillIconCatalogRefreshing = true;
    if (button) button.disabled = true;
    if (status) status.textContent = "最新のアイコン一覧を取得しています…";
    let requestUrl = SKILL_ICON_PAGE_URL;
    try {
      const url = new URL(SKILL_ICON_PAGE_URL, location.href);
      url.searchParams.set("_fsc_icon_refresh", String(Date.now()));
      requestUrl = url.href;
    } catch (error) {}
    return fetch(requestUrl, { credentials: "same-origin", cache: "no-store" })
      ...
      .then((html) => {
        const catalog = parseSkillIconCatalog(html);
        if (!catalog.length) throw new Error("empty catalog");
        skillIconCatalogCache = catalog;
        skillIconCatalogPromise = Promise.resolve(catalog);
        if (filter) filter.value = "";
        renderSkillIconCatalog(catalog);
        refreshSkillIconPreviews(document, catalog);
        return catalog;
      })
      .catch(() => {
      ...
      .finally(() => {
        skillIconCatalogRefreshing = false;
        if (button) button.disabled = false;
      });
  }

  function bindSkillIconPickerEvents() {
    if (skillIconEventsBound || typeof document === "undefined") return;
    skillIconEventsBound = true;
    document.addEventListener("click", (event) => {
      const open = event.target.closest("[data-skill-icon-open]");
      if (open) {
        const picker = open.closest("[data-skill-icon-picker-for]");
        if (picker) openSkillIconModal(picker);
        return;
      }
      const choice = event.target.closest("[data-skill-icon-value]");
      if (choice && activeSkillIconPicker) {
        setSelectedSkillIcon(choice.dataset.skillIconValue || "");
        closeSkillIconModal();
        return;
      }
      ...
      if (clear) {
        ...
        hidden.value = "";
        hidden.dispatchEvent(new Event("change", { bubbles: true }));
        updateSkillIconPreview(picker, skillIconCatalogCache || []);
        return;
      }
      const refreshButton = event.target.closest("[data-skill-icon-refresh]");
      if (refreshButton) {
        event.preventDefault();
        refreshSkillIconCatalog();
        return;
      }
      if (event.target.closest("[data-skill-icon-close]")) closeSkillIconModal();
    });
    document.addEventListener("input", (event) => {
      const filter = event.target.closest("[data-skill-icon-filter]");
      if (filter) filterSkillIconCatalog(filter.value);
    });
    document.addEventListener("keydown", (event) => {
      if (event.key === "Escape" && skillIconModalElement && !skillIconModalElement.hidden) closeSkillIconModal();
    });
  }
  ...
  function effectPreviewLine(effect) {
    ...
    if (item.effectType === "nobleAttack") {
      ...
      return `宝具攻撃（${target}・${item.attackStrength === "upgraded" ? "強化後倍率" : "強化前倍率"}）${formatEffectChance(item.chance)}`;
    }
    const specialNotation = item.valueMode === "oc5" ? "<OC:特攻威力UP>" : item.valueMode === "np5" ? "[Lv]" : "";
    const text = item.effectType === "specialAttack"
      ? `＆〔${cleanInline(item.specialTarget)}〕特攻${specialNotation}${formatEffectChance(item.chance)}`
      : `${item.prefix}${cleanInline(item.text)}${formatEffectChance(item.chance)}`;
  ...
  function effectTokens(kind) {
    if (kind === "class" || kind === "bond") return [];
  ...
  function effectEditor(effect, path, listPath, index, total, kind, scrollTarget) {
    ...
    const chanceControl = `${modelNumber(`${path}.chance`, "確率（％）", normalized.chance, "")}
      <p class="fsc-help">100以下は通常文字、101以上は赤字として効果文の最後尾へ追加します。</p>`;
    ...
    return `<details class="fsc-effect-row fsc-effect-tone-${index % 3}" open>
      ...
      ${normalized.raw ? raw : `${editor}${chanceControl}`}
    </div></details>`;
  }
  ...
  function preUpgradeEffectCopyButton(path) {
    if (!/\.upgraded$/.test(String(path || ""))) return "";
    return `<div class="fsc-entry-actions fsc-copy-effects"><button type="button" class="fsc-btn sub" data-content-action="copyPreUpgradeEffects" data-enabled-path="${path}.enabled">強化前の効果を複製</button></div>`;
  }
  ...
  function skillVersionEditor(version, path, effectTitle, scrollTarget) {
    ...
    return `${effectPreview(item.effects, `${path}.effects`)}<div class="fsc-grid">
      ...
      ${skillIconPicker(`${path}.icon`, "スキルアイコン", item.icon)}
      ${modelText(`${path}.ct`, "CT", item.ct, "")}
    </div>${preUpgradeEffectCopyButton(path)}<h5>${effectTitle || "効果"}</h5>${effectListEditor(item.effects, `${path}.effects`, "skill", scrollTarget)}`;
  }
  ...
  function skillVariantEditor(variant, path) {
    ...
    return `<details class="fsc-variant-tab" id="${anchor}" open><summary>${label}</summary><div class="fsc-variant-body">
      ...
      <div class="fsc-grid">
        ...
        ${changesIcon ? skillIconPicker(`${path}.icon`, "差分のスキルアイコン", item.icon) : ""}
  ...
  function classSkillEditor(skill, index, total) {
    ...
    return `<details class="fsc-entry fsc-entry-tone-${index % 4}" id="${anchor}" open><summary>クラススキル${index + 1}</summary><div class="fsc-entry-body">
      ...
      <div class="fsc-grid">${modelText(`classSkills.${index}.name`, "名称・ランク", item.name || "", "")}${skillIconPicker(`classSkills.${index}.icon`, "スキルアイコン", item.icon || "")}</div>
  ...
  function nobleVersionEditor(version, path, title, scrollTarget) {
    ...
    </div>${preUpgradeEffectCopyButton(path)}<h5>効果</h5>${effectListEditor(item.effects, `${path}.effects`, "noble", scrollTarget)}`;
  }
  ...
  function renderContentSettingsEditors(settings) {
    ...
    return {
      ...
      bond: `<details class="fsc-entry fsc-entry-tone-0" id="${bondAnchor}" open><summary>絆礼装</summary><div class="fsc-entry-body">${effectPreview(settings.bond.effects, "bond.effects")}<div class="fsc-grid">
        ${modelText("bond.name", "礼装名", settings.bond.name, "")}${skillIconPicker("bond.icon", "効果アイコン", settings.bond.icon)}
      </div>${modelCheckbox("bond.includeFieldCondition", "「自身がフィールドにいる間、」を含める", settings.bond.includeFieldCondition, false)}<h5>効果</h5>${effectListEditor(settings.bond.effects, "bond.effects", "bond", bondAnchor)}</div></details>`
    };
  }
  ...
  function installStyle() {
    ...
    style.textContent = `
      ...
      .fsc-gender-settings{grid-column:1/-1}.fsc-gender-list{display:flex;flex-direction:column;gap:8px}.fsc-gender-entry{display:grid;grid-template-columns:minmax(140px,180px) minmax(190px,1fr) auto;gap:8px;align-items:end;padding:8px;border:1px solid #d4e0eb;border-radius:7px;background:#f8fbfe}.fsc-gender-entry>.fsc-mini{margin-bottom:2px}
      ...
      #fgo-status-calculator .fsc-skill-icon-field{grid-column:1/-1}
      #fgo-status-calculator .fsc-icon-selected{display:flex;align-items:center;gap:10px;border:1px solid #c7d6e3;border-radius:8px;padding:8px;background:#fff;min-height:66px}
      #fgo-status-calculator .fsc-icon-selected-image{width:52px;height:52px;display:grid;place-items:center;border:1px solid #d7e2eb;border-radius:7px;background:#f8fafc;flex:0 0 52px}
      #fgo-status-calculator .fsc-icon-selected-image img{max-width:48px;max-height:48px}
      #fgo-status-calculator .fsc-icon-selected-meta{display:flex;flex-direction:column;gap:2px;min-width:0;flex:1}
      #fgo-status-calculator .fsc-icon-selected-meta strong{overflow-wrap:anywhere}
      #fgo-status-calculator .fsc-icon-selected-label{font-size:11px;color:#64748b}
      #fgo-status-calculator .fsc-icon-selected-actions{display:flex;gap:6px;flex-wrap:wrap;justify-content:flex-end}
      html.fsc-icon-modal-open{overflow:hidden}
      .fsc-icon-modal[hidden]{display:none!important}
      .fsc-icon-modal{position:fixed;inset:0;z-index:2147483000;display:grid;place-items:center;padding:20px;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI","Yu Gothic UI",Meiryo,sans-serif;color:#1f2937}
      .fsc-icon-modal *{box-sizing:border-box}
      .fsc-icon-modal-backdrop{position:absolute;inset:0;background:rgba(15,23,42,.58);backdrop-filter:blur(2px)}
      .fsc-icon-modal-panel{position:relative;width:min(1060px,calc(100vw - 32px));max-height:min(84vh,900px);display:flex;flex-direction:column;background:#fff;border:1px solid #aebfd0;border-radius:12px;box-shadow:0 18px 55px rgba(15,23,42,.35);padding:14px;overflow:hidden}
      .fsc-icon-modal-head{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;border-bottom:1px solid #d7e2eb;padding-bottom:10px}
      .fsc-icon-modal-head strong{font-size:17px}.fsc-icon-modal-current{font-size:12px;color:#64748b;margin-top:3px;overflow-wrap:anywhere}
      .fsc-icon-modal-close{border:0;background:transparent;font-size:28px;line-height:1;cursor:pointer;color:#475569;padding:0 5px}
      .fsc-icon-picker-tools{display:flex;align-items:center;gap:8px;margin:10px 0 5px}.fsc-icon-picker-tools input{min-width:0;flex:1;width:auto;border:1px solid #aebfd0;border-radius:6px;padding:9px;background:#fff;font:inherit}.fsc-icon-refresh{white-space:nowrap}.fsc-icon-refresh:disabled{opacity:.55;cursor:wait}
      .fsc-icon-picker-status{font-size:12px;color:#64748b;margin:4px 0 8px}.fsc-icon-picker-catalog{overflow:auto;padding-right:4px}
      .fsc-icon-category{margin:9px 0 13px}.fsc-icon-category h4{margin:0 0 6px;font-size:13px;color:#334155;position:sticky;top:0;background:#fff;padding:4px 0;z-index:1}
      .fsc-icon-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(92px,1fr));gap:6px}
      .fsc-icon-choice{display:flex;flex-direction:column;align-items:center;gap:4px;min-height:88px;border:1px solid #ccd9e5;border-radius:7px;background:#fff;padding:5px;cursor:pointer;color:#334155}
      .fsc-icon-choice:hover{border-color:#477fad;background:#f2f7fb}.fsc-icon-choice.is-selected{border:2px solid #35698f;background:#eaf4fb;padding:4px}.fsc-icon-choice img{width:48px;height:48px;object-fit:contain}.fsc-icon-choice span{font-size:10px;line-height:1.2;overflow-wrap:anywhere;text-align:center}
      @media(max-width:700px){.fsc-shell{padding:10px}.fsc-split{grid-template-columns:1fr}.fsc-section{padding:10px}.fsc-command-cards,.fsc-command-hits{grid-template-columns:repeat(2,minmax(120px,1fr))}.fsc-duration-tools{grid-template-columns:1fr 1fr}.fsc-duration-buttons{grid-column:1/-1}.fsc-trait-restriction-fields{grid-template-columns:1fr}.fsc-gender-entry{grid-template-columns:1fr}#fgo-status-calculator .fsc-icon-selected{align-items:flex-start;flex-wrap:wrap}#fgo-status-calculator .fsc-icon-selected-actions{width:100%;justify-content:flex-start}.fsc-icon-modal{padding:8px}.fsc-icon-modal-panel{width:calc(100vw - 16px);max-height:92vh}.fsc-icon-picker-tools{align-items:stretch;flex-direction:column}.fsc-icon-grid{grid-template-columns:repeat(auto-fill,minmax(78px,1fr))}}
    `;
  ...
  function renderApp(root) {
    ...
    root.innerHTML = `<form class="fsc-shell" autocomplete="on">
      ...
      <section class="fsc-section"><h3>基本設定</h3><div class="fsc-grid">
        ...
        <div class="fsc-gender-settings" data-gender-settings></div>
      </div></section>
      ...
      <section class="fsc-section"><h3>クラススキル設定</h3>
        <p class="fsc-help">名称・ランクを入力し、スキルアイコンは一覧から選択します。必要な数だけ追加し、上下ボタンで並べ替えられます。Lv.／OC表記は効果文へ直接入力します。</p>
        <div data-content-editor="classSkills"></div>
      </section>
      <section class="fsc-section"><h3>保有スキル設定</h3>
        <p class="fsc-help">Skill1～3の通常・強化後と、第二／第三再臨以降の差分を設定します。強化後を有効にすると強化前の効果を自動で追加します。スキルアイコンは検索可能な一覧から選択できます。</p>
        <div data-content-editor="skills"></div>
      </section>
      <section class="fsc-section"><h3>宝具設定</h3>
        <p class="fsc-help">宝具は1枠です。通常の宝具種類・Hit数と、強化後、第二／第三再臨以降の名称・効果・宝具種類・Hit数の差分をまとめて設定します。強化後を有効にすると強化前の効果を自動で追加します。効果の種類から「宝具攻撃」「特攻攻撃」を選ぶと、カード表記と倍率行を生成できます。</p>
    ...
    let genderState = normalizeGenderEntries(null, "男性");

    function renderGenderEditor() {
      const container = query("[data-gender-settings]");
      if (container) container.innerHTML = genderEntriesEditor(genderState);
    }

    function updateGenderControl(control) {
      const index = Number(control && control.dataset ? control.dataset.genderIndex : -1);
      const fieldName = control && control.dataset ? control.dataset.genderField : "";
      if (!Number.isInteger(index) || index < 0 || index >= genderState.length) return false;
      if (fieldName === "gender") genderState[index].gender = GENDERS.includes(control.value) ? control.value : GENDERS[0];
      else if (fieldName === "note") genderState[index].note = control.value;
      else return false;
      return true;
    }
    ...
    function renderContentEditors() {
      ...
      loadSkillIconCatalog().then((catalog) => refreshSkillIconPreviews(root, catalog));
    }
    ...
    const getInput = () => ({
      ...
      personality: query('[name="personality"]').value, gender: genderState[0].gender, genders: normalizeGenderEntries(genderState),
    ...
    function restoreState() {
      try {
        ...
        genderState = normalizeGenderEntries(state.genders, state.gender);
        Object.entries(state).forEach(([name, value]) => {
          if (name === "contentSettings" || name === "traitRestrictions" || name === "genders" || name === "gender") return;
    ...
    function updateContentControl(control) {
      ...
      if (value === true && /\.upgraded\.enabled$/.test(path)) {
        const effectsPath = path.replace(/\.enabled$/, ".effects");
        const currentEffects = modelValue(effectsPath);
        if (!Array.isArray(currentEffects) || !currentEffects.length) {
          setModelValue(effectsPath, preUpgradeEffects(contentState, path));
        }
      }
    ...
    root.addEventListener("click", (event) => {
      const genderButton = event.target.closest("[data-gender-action]");
      if (genderButton) {
        const action = genderButton.dataset.genderAction;
        if (action === "add") genderState.push({ gender: GENDERS[0], note: "" });
        else if (action === "remove" && genderState.length > 1) genderState.splice(Number(genderButton.dataset.genderIndex), 1);
        else return;
        renderGenderEditor();
        saveState();
        refresh();
      ...
      } else if (action === "copyPreUpgradeEffects") {
        const enabledPath = button.dataset.enabledPath || "";
        if (!window.confirm("現在の強化後効果をすべて削除し、強化前の効果で置き換えます。よろしいですか？")) return;
        if (!replaceUpgradedEffects(contentState, enabledPath)) return;
    ...
    root.addEventListener("change", (event) => {
      if (event.target.matches("[data-model-path]")) return;
      if (event.target.matches("[data-gender-index][data-gender-field]")) {
        updateGenderControl(event.target);
        saveState();
        refresh();
        return;
      }
