(function () {
  ...
  const TRAIT_GROUPS = [
    { label: "基本", traits: ["サーヴァント", "人型", "ギリシャ神話系男性"] },
    { label: "追加属性", traits: ["狂", "星", "夏", "花嫁", "星の力"] },
  ...
  const DEFAULT_TRAITS = ["サーヴァント", "人型"];
  const DERIVED_TRAITS = new Set([
    "男性", "女性", "性別不明", "秩序", "中立", "混沌", "善", "中庸", "悪", "獣",
    "天の力", "地の力", "人の力", "獣の力",
...
`.trim();
  ...
  function parseSelectedTraitsFromSource(source) {
    ...
    const selected = traitCell.split("/")
  ...
  function buildTraitLine(line, result) {
    ...
    const traits = normalizeSelectedTraits(result.input.selectedTraits);
    const derived = [result.input.gender === "-" ? "性別不明" : result.input.gender, result.input.policy];
    if (!["狂", "星", "夏", "花嫁"].includes(result.input.personality)) derived.push(result.input.personality);
    if (result.input.affinity !== "星") derived.push(`${result.input.affinity}の力`);
  ...
  const core = { VERSION, SERVANT_TEMPLATE, calculate, replaceSource, parseSelectedTraitsFromSource, buildColumnStyle, buildHeaderLine, buildStatusRow, buildNaLine, buildParameterRow };
  ...
  function renderApp(root) {
    ...
    root.innerHTML = `<form class="fsc-shell" autocomplete="on">
      ...
      <section class="fsc-section"><h3>基本設定</h3><div class="fsc-grid">
        ${field("No.", textInput("servantNo", "例：068"))}
        ${field("真名", textInput("trueName", "例：ヘリオガバルス"))}
      ...
      <section class="fsc-section"><h3>特性</h3>
        <p class="fsc-help">該当する特性を複数選択できます。性別・方針・性格・相性・クラス由来の特性は、基本設定から自動入力されます。</p>
    ...
    function refresh() {
      ...
      const omittedClasses = new Set(["アーチャー", "ランサー", "アサシン", "シールダー", "アヴェンジャー", "フォーリナー", "プリテンダー"]);
      if (!omittedClasses.has(result.classData.name)) derived.push(result.classData.name);
      query("[data-derived-traits]").textContent = `基本設定から自動入力：${derived.join(" / ")}`;
