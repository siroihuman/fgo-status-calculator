...
`.trim();
  ...
  function emptySkillVariant(stage) {
    return Object.assign({ enabled: false, stage: String(stage), changeMode: "name" }, emptySkillVersion());
  }
  ...
  function emptyNobleVariant(stage) {
    return Object.assign({ enabled: false, stage: String(stage), changeMode: "name" }, emptyNobleVersion());
  }
  ...
  function skillVersionEditor(version, path, effectTitle) {
    ...
    return `<div class="fsc-grid">
      ${modelText(`${path}.name`, "名称・ランク", item.name, "例：対魔力 A")}
      ${modelText(`${path}.icon`, "アイコンファイル名", item.icon, "例：skill-attack-up.png")}
      ${modelText(`${path}.ct`, "CT", item.ct, "例：8")}
  ...
  function classSkillEditor(skill, index, total) {
    ...
    return `<details class="fsc-entry" open><summary>クラススキル${index + 1}</summary><div class="fsc-entry-body">
      ...
      <div class="fsc-grid">${modelText(`classSkills.${index}.name`, "名称・ランク", item.name || "", "例：対魔力 A")}${modelText(`classSkills.${index}.icon`, "アイコンファイル名", item.icon || "", "例：対魔力.png")}</div>
  ...
  function nobleVersionEditor(version, path, title) {
    ...
    return `<h4>${title}</h4><div class="fsc-grid">
      ${modelText(`${path}.reading`, "ルビ", item.reading, "例：エクスカリバー")}
      ${modelText(`${path}.name`, "正式名称", item.name, "例：約束された勝利の剣")}
      ${modelText(`${path}.rank`, "ランク", item.rank, "例：A++")}
      ${modelText(`${path}.category`, "種別", item.category, "例：対城宝具")}
  ...
  function renderContentSettingsEditors(settings) {
    return {
      ...
      bond: `<details class="fsc-entry" open><summary>絆礼装</summary><div class="fsc-entry-body"><div class="fsc-grid">
        ${modelText("bond.name", "礼装名", settings.bond.name, "")}${modelText("bond.icon", "効果アイコンファイル名", settings.bond.icon, "例：skill-card-arts-up.png")}
