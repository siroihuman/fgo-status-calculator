...
`.trim();
  ...
  const core = {
    ...
    createDefaultContentSettings, normalizeContentSettings, preUpgradeEffects, replaceUpgradedEffects, buildOwnedSkillsSection, buildNobleSection, formatEffectSuffix, insertEffectNotation, toggleEffectNotation, effectTokens, renderContentSettingsEditors,
  ...
  function installStyle() {
    ...
    style.textContent = `
      ...
      @media(max-width:700px){.fsc-shell{padding:10px}.fsc-split{grid-template-columns:1fr}.fsc-section{padding:10px}.fsc-command-cards,.fsc-command-hits{grid-template-columns:repeat(2,minmax(120px,1fr))}.fsc-duration-tools{grid-template-columns:1fr 1fr}.fsc-duration-buttons{grid-column:1/-1}.fsc-trait-restriction-fields{grid-template-columns:1fr}#fgo-status-calculator .fsc-icon-selected{align-items:flex-start;flex-wrap:wrap}#fgo-status-calculator .fsc-icon-selected-actions{width:100%;justify-content:flex-start}.fsc-icon-modal{padding:8px}.fsc-icon-modal-panel{width:calc(100vw - 16px);max-height:92vh}.fsc-icon-picker-tools{align-items:stretch;flex-direction:column}.fsc-icon-grid{grid-template-columns:repeat(auto-fill,minmax(78px,1fr))}}
    `;
